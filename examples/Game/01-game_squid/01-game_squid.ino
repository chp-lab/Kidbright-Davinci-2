...
#define SWITHCH_left 34
#define SWITHCH_right 35
...
void setup() {
  ...
  pinMode(23, OUTPUT);
  pinMode(25, OUTPUT);
  pinMode(SWITHCH_right, INPUT);
  pinMode(SWITHCH_left, INPUT);
  display.init();
...
void loop() {
  ...
  if (display_start_game == 0)
  {
    ...
    if (digitalRead(SWITHCH_right) == 1 || digitalRead(SWITHCH_left) == 1) {
      RGB.begin();
      RGB.red();
